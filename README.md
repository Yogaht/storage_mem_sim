...
- Python ≥ 3.10，建议通过 conda 创建环境
- 编译 Ramulator2 需要 C++17 编译器（macOS: `brew install gcc@14`）

```bash
conda create -n memsim python=3.13 -y
conda activate memsim
```

### 安装依赖

```bash
pip install -e .            # 安装 pyyaml 等项目依赖
...
```bash
python -m pytest            # 全部测试（需先完成 Ramulator2 安装）
